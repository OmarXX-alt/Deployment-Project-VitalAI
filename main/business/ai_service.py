...
def get_reaction(
    ...
    try:
        ...
        prompt = (
            ...
            "Respond with ONLY valid JSON (no markdown, no extra text) with fields: "
            "type (string: 'positive'|'neutral'|'alert'), "
            "message (1-2 sentence encouragement), "
            "tags (array of 1-2 relevant tags). Example: "
            '{"type":"positive","message":"Great job!","tags":["consistency"]}'
        )
...
def get_wellness_insights(
    ...
    try:
        ...
        slim_context = {
            ...
            "meals_avg_kcal": (
                round(meals.get("avg_calories", 0)) if has_meal_data else None
            ),
            "workouts_count": workouts.get("sessions", 0),
            "workouts_total_min": (
                workouts.get("total_minutes", 0) if has_workout_data else None
            ),
            "sleep_entries": sleep.get("entries", 0),
            "sleep_avg_hours": (
                round(sleep.get("avg_duration_hours", 0) * 10) / 10
                if has_sleep_data
                else None
            ),
            "sleep_avg_quality": (
                round(sleep.get("avg_quality", 0) * 10) / 10
                if has_sleep_data and sleep.get("avg_quality")
                else None
            ),
            ...
            "mood_avg_score": (
                round(mood.get("avg_score", 0) * 10) / 10
                if has_mood_data
                else None
            ),
            "note": (
                "If no data logged for a category, suggest user start tracking it."
            ),
        }
        ...
        try:
            ...
            if "{" in response and "}" in response:
                ...
                logger.debug(
                    f"Extracted JSON from response: {response_clean[:200]}"
                )
        ...
        return {
            ...
            if positives
            else ["Start tracking your health data"],
            "concern": str(concern)
            if concern
            else "Limited health data this week",
            "suggestions": suggestions[:3]
            if suggestions
            else ["Log your meals", "Track workouts", "Monitor sleep"],
...
def get_chat_response(
    ...
    try:
        ...
        prompt = (
            ...
            f"Chat history:\n{history_text}\n"
            f"User: {message}\n\n"
            "Reply empathetically in 2-4 sentences. Be supportive and actionable."
        )
