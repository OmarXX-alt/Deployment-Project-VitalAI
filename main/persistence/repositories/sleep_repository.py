...
def insert(doc: dict) -> str:
    try:
        ...
        raise RuntimeError("Failed to insert sleep log") from exc


def find_by_id(doc_id: str) -> dict | None:
    try:
        doc = get_db().sleep_logs.find_one({"_id": _to_object_id(doc_id)}, _SLEEP_PROJECTION)
        return _serialize_value(doc) if doc else None
    except Exception as exc:
        raise RuntimeError("Failed to find sleep log by id") from exc


def find_by_user(user_id: str) -> list[dict]:
    try:
        cursor = get_db().sleep_logs.find({"user_id": _to_object_id(user_id)}, _SLEEP_PROJECTION)
        return _serialize_docs(list(cursor))
    except Exception as exc:
        raise RuntimeError("Failed to find sleep logs by user") from exc
...
def find_recent(user_id: str, days: int = 7) -> list[dict]:
    try:
        ...
        raise RuntimeError("Failed to find recent sleep logs") from exc
...
def update_ai_reaction(log_id: str, reaction: dict) -> bool:
    try:
        ...
        raise RuntimeError("Failed to update sleep ai_reaction") from exc


def delete_by_id(doc_id: str) -> bool:
    try:
        result = get_db().sleep_logs.delete_one({"_id": _to_object_id(doc_id)})
        return result.deleted_count == 1
    except Exception as exc:
        raise RuntimeError("Failed to delete sleep log by id") from exc
